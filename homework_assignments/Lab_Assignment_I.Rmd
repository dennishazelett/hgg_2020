...
knitr::opts_chunk$set(root.dir = '~/repos/hgg_2020')
```
...
**Important Note: DO NOT INSTALL R version 4. Although it contains an official release, it contains a major flaw in that a previous default setting for a base package has been changed and is likely to break many dependent packages.**
...
3. Notice that this model matrix has 3 columns.
...
metadat <- read.csv(file = '~/repos/hgg_2020/data/crispri_modelmatrix.csv', header = TRUE)
...
cts <- read.csv('~/repos/hgg_2020/data/combinedfeatcounts.csv', header = TRUE)
## Take a quick peak at the contents
knitr::kable(cts[1:5, 1:4])

## We need to select the columns from our tead.samples, including the Geneid column:

tead.counts <- subset(cts, select = c(1, tead.samples))

## Now we're ready to export these object for upload to GENAVi
write.table(
  tead.counts, 
  file = "~/repos/hgg_2020/data/teadfeatcounts.csv",
  quote = FALSE
)

write.table(
  tead.metadat,
  file = "~/repos/hgg_2020/data/teadmodel.csv",
  quote = FALSE
)
```
